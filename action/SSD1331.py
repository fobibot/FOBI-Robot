...
class SSD1331:
    ...
    def Emotion(self,data):
        self.DrawFullScreenBitMap(self.emotions[data][0])
        return
    def Blink(self,data):
        for x in self.emotions[data]:
            self.DrawFullScreenBitMap(x)
        return
...
if __name__ == '__main__':
    ...
    try:
        ...
        while True:
            device.Animate('normal')
            time.sleep(2)
            device.Animate('blink')
            for x in range(10):
                device.Animate('heart')
